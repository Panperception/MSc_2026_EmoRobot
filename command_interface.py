...
class CommandInterface(object):
    ...
    def __init__(self, function_call_response_callback=None):
        ...
        self.interfaces = {
            ...
            "perform_gesture": self._cmd_perform_gesture,
            "perform_emotion": self._cmd_perform_emotion,
            "perform_both": self._cmd_perform_both,       
   ...
   # look command 
    ...
    def _cmd_perform_gesture(self, params):
            gesture = params.get('gesture', '')
            block = params.get('block', False)
            if block:
                rospy.loginfo(f"Performing gesture: {gesture} (blocking)")
                self.play_gesture(gesture)
            else:
                rospy.loginfo(f"Performing gesture: {gesture}")
                thread = threading.Thread(target=self.play_gesture, args=(gesture,))
                thread.start()
    
    def _cmd_perform_emotion(self, params):
            emotion = params.get('emotion', '')
            block = params.get('block', False)
            #check if the LLM says to block the sentence
            if block:
                rospy.loginfo(f"Showing emotion: {emotion} (blocking)")
                self.show_emotion(emotion)
            else:
                rospy.loginfo(f"Showing emotion: {emotion}")
                thread = threading.Thread(target=self.show_emotion, args=(emotion,))
                thread.start()
        
    def _cmd_perform_both(self, params):
        gesture = params.get('gesture', '')
        expression = params.get('expression', '')
        block = params.get('block', False)
        if block:
            rospy.loginfo(f"Performing gesture: {gesture} and emotion: {expression} (blocking)")
            self.play_gesture(gesture)
            self.show_emotion(expression)
        else:
            thread1 = threading.Thread(target=self.play_gesture, args=(gesture,))
            thread2 = threading.Thread(target=self.show_emotion, args=(expression,))
    ...
    def _cmd_end_conversation(self, params):
        rand_sendoff = random.choice(self.rand_sendoffs)
        rospy.loginfo("Ending conversation")
        self.talkText(rand_sendoff)
        rospy.signal_shutdown("User has requested end of conversation")
...
# set language and voice
    ...
    def show_emotion(self, emotion: str):
        status = self.emotionShow("QT/"+ emotion)
    
    def play_gesture(self, gesture: str):
        status = self.gesturePlay("QT/"+ gesture, 0)
